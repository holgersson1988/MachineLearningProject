...
int main()
{
	...
	// Set up players
	char player1Char = 254;
	char player2Char = 15;
	char player = player2Char;//start as player 2 will change back 2 player 1
	...
	while (!gamewon){//will stop when game is won, ! means NOT makes the oppisite be checked
		if (hold2 != -1){//check if there was a error in the last drop
			if (player == player2Char){//if player 2 lasted dropped a piece so its player 1s turn
				cout << "player 1 drop where?";
				player = player1Char;//char of players piece
			}
			else{
				cout << "player 2 drop where?";
				player = player2Char;//char of player piece
			}
		}
	...
	if (player == player2Char)//if won by player 2
